...
import "./openzeppelin-new/ReentrancyGuardNew.sol";
import "./openzeppelin-new/SafeERC20New.sol";
 ...
 */
contract TokenDistributorNew is ReentrancyGuardNew {
    using SafeERC20New for ILooksRareToken;
    ...
    // error
    error WrongRewardParams(); // Wrong reward parameters

    error DepositAmountEqZero(); // Deposit: Amount must be > 0

    error WithdrawAmountBeyondRange(); // Withdraw: Amount must be > 0 or lower than user balance

    error WithdrawAmountEqZero(); // Withdraw: Amount must be > 0
    ...
    constructor(
        ...
        // TODO , this means all lookrares token will be distribute in this contract
