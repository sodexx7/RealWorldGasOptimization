...
describe("TokenDistributorNew", () => {
  ...
  beforeEach(async () => {
    [admin, user1, user2, user3, user4, tokenSplitter] = await ethers.getSigners();
    ...
    startBlock = BigNumber.from((await ethers.provider.getBlockNumber()).toString()).add("100");
    ...
    const TokenDistributor = await ethers.getContractFactory("TokenDistributorNew");
    ...
    for (const user of [user1, user2, user3, user4]) {
      await looksRareToken.connect(admin).transfer(user.address, parseEther("500"));
  ...
  describe("#1 - Regular user/admin interactions", async () => {
    it("Scenario 1 - Block rewards change automatically with 4 halvings at a fixed frequency", async () => {
      ...
      assert.deepEqual(await tokenDistributor.calculatePendingRewards(user1.address), parseEther("933.75"));
      ...
      await expect(tx)
        ...
        .withArgs("1", startBlock.add("100"), parseEther("15"), parseEther("35"));

      // (100 * 70) + (50 * 35) = 8,750 tokens for token splitter (EOA for this test)
      assert.deepEqual(await looksRareToken.balanceOf(tokenSplitter.address), parseEther("8750"));

      await advanceBlockTo(startBlock.add("199"));

      // 937.5 + 49 * 15/3 = 1182.5
      assert.deepEqual(await tokenDistributor.calculatePendingRewards(user2.address), parseEther("1182.5"));
      ...
      // 1187.5 + 50 = 1237.5
      assert.deepEqual((await tokenDistributor.userInfo(user2.address))[0], parseEther("1237.5"));

      // (100 * 70) + (100 * 35) = 10,500 tokens for token splitter (EOA for this test)
      assert.deepEqual(await looksRareToken.balanceOf(tokenSplitter.address), parseEther("10500"));
      ...
      await expect(tx).to.emit(tokenDistributor, "Compound").withArgs(user2.address, parseEther("322.8260869560375"));

      await expect(tx)
        .to.emit(tokenDistributor, "NewRewardsPerBlock")
        .withArgs("2", startBlock.add("200"), parseEther("7.5"), parseEther("17.5"));

      assert.deepEqual((await tokenDistributor.userInfo(user2.address))[0], parseEther("1560.326086956037500000"));
      ...
      await expect(tx)
        .to.emit(tokenDistributor, "NewRewardsPerBlock")
        .withArgs("3", startBlock.add("300"), parseEther("3.75"), parseEther("8.75"));
      ...
      assert.deepEqual(await looksRareToken.balanceOf(tokenSplitter.address), parseEther("13125"));
    ...
    it("Scenario 2 - Block rewards change automatically with 4 adjustments at a non-fixed frequency", async () => {
      ...
      const rewardsPerBlockForStaking = [parseEther("120"), parseEther("30"), parseEther("7.5"), parseEther("3.75")];
      const rewardsPerBlockForOthers = [parseEther("280"), parseEther("70"), parseEther("17.5"), parseEther("8.75")];
      const periodLengthesInBlocks = [
        BigNumber.from("25"),
        BigNumber.from("50"),
        BigNumber.from("100"),
      ...
      assert.deepEqual(await tokenDistributor.calculatePendingRewards(user1.address), parseEther("930"));
      ...
      // 1468.75 + 50 = 1518.75
      assert.deepEqual((await tokenDistributor.userInfo(user2.address))[0], parseEther("1518.75"));

      // 280 * 25 + 70 * 50 + 17.5 * 100 + 8.75 * 25 = 12,468.75 tokens for token splitter (EOA for this test)
      assert.deepEqual(await looksRareToken.balanceOf(tokenSplitter.address), parseEther("12468.75"));
      ...
      assert.deepEqual((await tokenDistributor.userInfo(user2.address))[0], parseEther("1767.272727272175"));
    ...
    it("User can unstake after 10 blocks", async () => {
      ...
      assert.deepEqual(await tokenDistributor.calculatePendingRewards(user1.address), parseEther("67.5"));
      ...
      assert.deepEqual(await looksRareToken.balanceOf(tokenSplitter.address), parseEther("700"));
    });
    ...
    it("Anyone can call updatePool", async () => {
      ...
      await expect(tx)
        ...
        .withArgs(constants.AddressZero, tokenDistributor.address, parseEther("300"));
    ...
    it("User can withdraw before start, deposit after start", async () => {
      assert.deepEqual(await tokenDistributor.calculatePendingRewards(user1.address), constants.Zero);

      for (const user of [user1, user2, user3]) {
        const tx = await tokenDistributor.connect(user).withdraw(parseEther("100"));
  ...
  describe("#2 - Revertions", async () => {
    it("Cannot set with wrong parameters", async () => {
      ...
      periodLengthesInBlocks = [BigNumber.from("100"), BigNumber.from("100"), BigNumber.from("100")];
      ...
      rewardsPerBlockForStaking = [parseEther("15"), parseEther("7.5"), parseEther("3.75")];
      ...
      // Reset
      rewardsPerBlockForStaking = [parseEther("30"), parseEther("15"), parseEther("7.5"), parseEther("3.75")];

      // Drop one of the items
      rewardsPerBlockForOthers = [parseEther("35"), parseEther("17.5"), parseEther("8.75")];
      ...
      // Reset
      rewardsPerBlockForOthers = [parseEther("70"), parseEther("35"), parseEther("17.5"), parseEther("8.75")];
      ...
      ).to.be.revertedWith("WrongRewardParams()");
      ...
      rewardsPerBlockForStaking = [parseEther("60"), parseEther("15"), parseEther("7.5"), parseEther("3.75")];
    ...
    it("Cannot deposit if amount is 0", async () => {
      await expect(tokenDistributor.connect(user1).deposit("0")).to.be.revertedWith("DepositAmountEqZero()");
    });
    ...
    it("Cannot withdraw if amount is 0 or larger than user balance", async () => {
      await expect(tokenDistributor.connect(user1).withdraw("0")).to.be.revertedWith(
        "Withdraw: Amount must be > 0 or lower than user balance"
      );

      await expect(tokenDistributor.connect(user1).withdraw(parseEther("100.0000001"))).to.be.revertedWith(
        "Withdraw: Amount must be > 0 or lower than user balance"
      );
